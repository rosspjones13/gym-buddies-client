...
import { mapsKey } from "../constants/keys";
...
class GymPage extends Component {
  ...
  filterCheckedUsers = () => {
    ...
    return allUsers.filter(
      (user) =>
        user.checkin === selectedMarker.id &&
  ...
  filterCheckedBuddies = () => {
    ...
    const checkedBuddies = userBuddies.filter(
      (user) => this.pickBuddy(user).checkin === selectedMarker.id
    );
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    patchUserCheckin: (user) => {
      dispatch(patchUserCheckin(user));
    },
    sendBuddyRequest: (user, currentUser) => {
